...
public final class FindMeetingQueryTest {
  ...
  public void OptionalAttendeeNotAvailable() {
    // Have two required attendees have different events. Have optional attendee that has an all day event.
    // We should see three options because each required person has split the restricted times,
    // and optional has no available time.
    //
    // Events  :       |--A--|     |--B--|
    //           |--------------C--------------|
    // Day     : |-----------------------------|
  ...
  public void OptionalAttendeeAvailable() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times, but all meeting are back to back.
    //
    // Events  :       |--A--|--C--|--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|                 |--2--|
  ...
  public void justEnoughRoomWithoutOptional() {
    // Have one person mandetory and one optional, but make it so that there is just enough room at
    // one point in the day to have the meeting without optional.
    //
    // Events  : |--A--|--B--|----A----|
    // Day     : |---------------------|
    // Options :       |-----|
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 2", TimeRange.fromStartEnd(TIME_0900AM, TimeRange.END_OF_DAY, true),
            Arrays.asList(PERSON_A)),
        new Event("Event 3", TimeRange.fromStartEnd(TIME_0845AM, DURATION_15_MINUTES, false),
            Arrays.asList(PERSON_B)));
  ...
  public void onlyOptionalAttendees() {
    // Have events for each person, their events overlap. We should only see two options.
    // Both are optional.
    //
    // Events  :       |--A--|--A--|
    //                     |--B--|
  ...
  public void notEnoughRoomForOptional() {
    // Have two people, both optional, but make it so that there is not enough room
    // at any point in the day to have the meeting, so should return all day because their optional.
    //
    // Events  : |--A-----| |-----B----|
    // Day     : |---------------------|
    // Options :
